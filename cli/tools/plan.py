...
class UpdatePlanTool(BaseTool):
    """The `update_plan` tool plus pure helpers for parsing/applying calls.

    The parsing/applying helpers are intentionally stateless so they can be
    reused by unit tests and by any future caller that wants to write a plan
    without going through the tool dispatcher.
    """
