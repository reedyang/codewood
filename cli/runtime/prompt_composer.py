...
def build_console_system_append(agent: Any) -> str:
    ...
    return (
        ...
        "- `console_send` writes arbitrary text/keystrokes with no Enter "
        "appended — use it to interact with interactive programs (gdb, "
        "debuggers, REPLs): answer a `(gdb)` prompt with e.g. `\"next\\r\"`, "
        "reply `y`/`n`, or send control keys such as `\\x03` (Ctrl+C) and "
        "`\\u001b[A` (Up arrow). Escape sequences are decoded by default; "
        "write `\\\\` for a literal backslash.\n"
        "- `console_wait` blocks until new output appears (or a timeout), "
        "then returns the new lines — without `start` it waits for output "
        "after the last `console_read`/`console_wait` position. Use it after "
        "`console_send`/`console_exec` to synchronize with interactive "
        "programs instead of polling `console_read`. With `stable` true it "
        "waits until the output settles (3s quiet), so one call awaits "
        "the final value of a progress bar / percent refresh stream.\n"
        "- `console_interrupt` sends Ctrl+C (SIGINT) to interrupt a running "
        "program or the debugger.\n"
        "- `console_resize` changes the terminal dimensions (needed for "
...
def build_tools_prompt_append(agent: Any) -> str:
    ...
    template = preprocess_prompt(template, {
        ...
        "user_preferences_enabled": str("user_preferences_read" not in disabled and "user_preferences_patch" not in disabled).lower(),
    })
