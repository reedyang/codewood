...
def handle_chat_edit_command(agent: Any, raw_index: str) -> None:
    ...
    with agent._chat_state_lock:
        ...
        try:
            ...
            pass
        # Drop file_changes.json entries whose [FILE_CHANGE_REF] messages
        # were truncated away, so reload doesn't show stale file changes.
        try:
            pruner = getattr(agent, "_prune_file_changes_sidecar", None)
            if callable(pruner):
                pruner()
        except Exception:
            pass
        # Drop shell_output_*.txt files orphaned by the truncation.
        try:
            pruner = getattr(agent, "_prune_shell_output_files", None)
            if callable(pruner):
                pruner()
        except Exception:
            pass
        ...
        try:
            ...
            pass
        # Drop backup files orphaned by the truncation.
        try:
            pruner = getattr(agent, "_prune_backup_files", None)
            if callable(pruner):
                pruner()
        except Exception:
            pass
