...
from src.agent import Agent, ASK_MORE_INFO_ANSWER_HISTORY_PREFIX


class AskMoreInfoAnswerHistoryTests(unittest.TestCase):
    """The recorded ask_more_info selection is a marked assistant message so
    ...
    def test_build_and_parse_round_trips(self):
        content = self.agent._build_ask_more_info_answer_history_content("Prod")
        self.assertTrue(content.startswith(ASK_MORE_INFO_ANSWER_HISTORY_PREFIX))
        self.assertEqual(
            self.agent._parse_ask_more_info_answer_history_content(content), "Prod"
        )

    def test_parse_returns_none_for_unrelated_content(self):
        self.assertIsNone(
            self.agent._parse_ask_more_info_answer_history_content("just some text")
        )
        self.assertIsNone(
            self.agent._parse_ask_more_info_answer_history_content("")
        )

    def test_record_appends_excluded_assistant_message(self):
        self.agent._record_ask_more_info_answer_history("A; C")
        ...
        self.assertEqual(
            self.agent._parse_ask_more_info_answer_history_content(msg["content"]),
            "A; C",
        )

    def test_record_ignores_empty_answer(self):
        self.agent._record_ask_more_info_answer_history("   ")
