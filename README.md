...
- 在提示符下，内置命令（退出、帮助、清屏、清空上下文、知识库、自由模式等）须以 **`/`** 开头，例如 `/exit`、`/help`、`/clear screen`、`/clear context`、`/knowledge on`。不经 AI、由程序直接执行的本机 shell 命令或脚本须以 **`!`** 开头，例如 `!dir`、`!git status`。
- 未加 `/` 的输入一律作为自然语言交给 AI。
...
- `knowledge_tools_enabled`: 是否开启知识库维护类工具（默认 `false`）。关闭时不可用：`knowledge_sync`、`knowledge_stats`
