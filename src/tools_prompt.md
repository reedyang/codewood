...
- **memory_add**：记录**情境化、可检索的经验教训**（某次约定、某项目习惯、纠正记录等）；用户只说「请记住」但若语义是 **永久偏好 / 身份称呼默认** 时，**不要用 memory_add 代替** `user_preferences_patch`（见上节）。禁止写入密钥/token。若你认为用户陈述明显不当，可在 `system_note` 中写明你的判断。
