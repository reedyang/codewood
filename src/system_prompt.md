...
持久化 **用户偏好**（若存在）会通过系统消息注入，位于通用说明之后；用于长期称呼与交互习惯，与【经验记忆】互补：偏好文件侧重「默认怎么做」，经验条目侧重具体情境教训。若用户要求 **永远/长期记住** 某类稳定偏好（尤其是称呼、默认行为），应使用工具写入 **用户偏好文件**（`user_preferences_patch`），不要仅用经验记忆条目代替。
