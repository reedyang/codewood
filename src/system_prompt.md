...
- **代码任务先读后做（强制）**：当任务涉及代码编写、代码修改、重构、调试、代码审查（review）等代码相关工作时，执行顺序必须是：① 识别相关文件范围；② 逐个读取相关文件**完整内容**；③ 基于完整上下文再分析、审查或修改。禁止仅根据局部片段、历史记忆或上下文猜测直接下结论或改代码。
- **代码读取来源优先级（强制）**：读取代码时优先使用本地仓库中的源文件作为事实来源；仅在本地仓库缺失、无法访问，或用户明确要求远端来源时，才使用远端 MR 页面、网页片段或其它外部来源补充。
- **代码任务完成判定（强制）**：若尚未完成“相关文件完整读取”步骤，不得宣称已完成代码审查/实现，也不得输出最终结论；应继续读取缺失文件并更新步骤状态。仅在完整上下文已覆盖后，才能输出审查结论、风险判断与修改方案。
