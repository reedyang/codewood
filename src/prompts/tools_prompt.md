...
2. Do NOT also emit the same list in your natural-language message (no "Please reply with 1-N" bullet list, no Markdown enumeration of the same items). The host renders the option buttons from `options`; printing the list twice clutters the chat and is redundant. Your message should describe the question/context only, then call `ask_more_info`.
