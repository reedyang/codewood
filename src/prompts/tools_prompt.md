...
- 在用户点名要求的字段未出现在回复正文前，禁止调用 `done`。

图片任务规则（强制）：

- 对 `read_image` 或其它需要图片理解的任务，必须先判断当前模型是否支持多模态输入。
- 若模型不支持多模态，必须直接输出不支持说明并结束。
...
用户若仅请求“查询指定 MCP 信息”，在完成该模板渲染后，下一步必须调用 `done`，
