...
- 回复正文不是工具调用通道；除非当前轮明确启用“模拟 JSON tool call 模式”，否则禁止在回复文本中输出任何工具调用 JSON、`<tool_calls>` 伪标签、Markdown 工具调用代码块或 `tool`/`args` 示例。
- 每一轮优先判断是否需要继续调用工具：
  - 需要继续执行：调用对应工具。
  - 用户目标已完成：调用 `done`。
...
- 多步任务标准输出模板（必须遵循）：
  - 正确示例：
    ...
    <按当前轮系统/运行时指令给出本轮唯一工具调用>
  - 下一轮正确示例（收到上一步结果后）：
    Step 1 [completed]: 获取 playwright MCP 最新状态  
    Step 2 [completed]: 输出状态报告并收尾
    <按当前轮系统/运行时指令给出完成调用（done）>

- 示例 1：仅作工具调用（示例格式以当前系统约束为准）
  - 正确示例：<按当前轮系统/运行时指令发起唯一工具调用>

- 示例 2：需要继续执行（示例格式以当前系统约束为准）
  - 正确示例：
    已获取到当前状态，下一步我将刷新 playwright 的实时状态。
    <按当前轮系统/运行时指令发起下一步工具调用>

- 示例 3：任务已完成（示例格式以当前系统约束为准）
  - 正确示例：
