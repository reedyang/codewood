...
4. **AI 决策** - 由AI分析决策
5. **数据源集成** - 通过标准多-skill编排由上游注入行情快照（推荐上游为 `baidu` skill）
...
不要在本 skill 内直接调用其他 skill 脚本。应由 AI 先调用 `baidu` skill，再把结构化行情快照注入本 skill：
...
- 禁止在本 skill 代码内通过 `subprocess/shell` 直接调用 `baidu` 或其他 skill 脚本；必须由上游 AI 先执行其他 skill，再通过 `--quote-json/--quote-file` 注入数据。
