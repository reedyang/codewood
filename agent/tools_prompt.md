...
- 先判断缺失信息是否可通过现有能力获取：内置 tools、已加载 skills、MCP tools/resources/prompts。
- 若可获取，必须先调用相应工具自行获取，禁止向用户提问。
- 仅当你已判断为“当前能力无法可靠获取”时，才允许调用 `ask_more_info`。
...
- **memory_search**：仅当该段**仍不足以**回答当前问题、且确实需要额外检索时再调用；若 system 已含足够信息，应直接自然语言作答并 `done`，不要为走流程而检索。
