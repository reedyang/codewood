...
> **能读 ≠ 能写**。读取能力取决于沙箱用户身份被授予的权限;工作区外
> 即使可读也不可写。沙箱不是防数据泄漏系统,也不等同于虚拟机。

## Windows 实现

与 Codex 相同,Windows 沙箱不是单个"沙箱程序",而是多个 Windows 原生
安全机制的组合。命令执行采用 **runner 进程** 架构(参考 Codex 的
`window-sandbox-rs`):主进程用 `CreateProcessWithLogonW` 以沙箱用户身份
启动一个 **runner**,runner 再为真实命令派生 **受限令牌**(restricted
token)并用 `CreateProcessAsUserW` 启动之:

```text
主进程 ──CreateProcessWithLogonW──▶ runner(沙箱用户)
...
runner 本身用**普通登录令牌**运行(故沙箱用户组对其可读即可);真实命令
...
5. **Job Object(`KILL_ON_JOB_CLOSE`)**:runner 把真实命令挂到 job 上;
   沙箱命令被中断或超时后,整棵进程树一起结束。
6. **DPAPI**:沙箱用户密码用 `CryptProtectData` 加密后存于
   `%LOCALAPPDATA%/<应用名>/sandbox/sandbox_secret.bin`。

> **共享状态目录**:沙箱的全部状态(密钥、预置标记、capability SID、
> 运行时目录、ACL 记录)都放在 `%LOCALAPPDATA%/<应用名>/sandbox/`,
> 与各 `config` 目录无关,同一台机器上的多个 Code Wood 实例共享同一套
> 沙箱用户与密码,重建用户不再互相影响。升级后首次运行预置会重新创建
...
设置页(Settings → Security)加载时会用 `LogonUserW` 校验两个沙箱用户
的密码是否与当前数据目录的密钥一致(60 秒缓存,避免重复失败登录触发
账户锁定);若用户存在但密码不一致,页面会提示并显示"设置沙箱"入口,
...
如果 `sandbox_level` 配置为 `read_only` / `workspace_write` 但沙箱尚未
预置,`shell` 命令会被拒绝并返回明确错误,而不会静默降级为无沙箱执行。
...
申请以完全权限执行该命令(仅本次,不改变沙箱配置)。执行前一定会弹出
确认,用户有三个选择:
...
- 该工具调用**跳过 AI 自动审核**(`_freedom_auto_confirm`),因为用户已在
  提权确认中人工把关;
- 仅当沙箱实际激活(`read_only` / `workspace_write` 且平台支持)时上述
  跳过才生效;`full_access` 或平台不支持时 `bypass_sandbox` 是空操作,
  AI 审核与常规确认照旧。

> 提权是**显式用户批准**的例外通道:它只影响被批准的那一条命令,不会
> 改变 `config.jsonc` 中的 `sandbox_level`,也不会让后续命令绕过沙箱。
...
- 沙箱只约束 AI 的 `shell` 工具;`apply_patch` / `read` / grep 等进程内
  文件工具仍由现有路径策略约束;GUI 嵌入式终端是用户直接操作,不沙箱。
- 读取范围取决于沙箱用户身份被授予的权限:用户个人目录(`C:\Users\...`)
  下的文件通常不可读(这正是期望的安全边界)。
- 受保护目录(`.git`、`.codewood`)在沙箱内不可写——例如 `git commit`
  等需要写 `.git` 的命令在沙箱级别下会失败,请切换到 `full_access`。
...
- 沙箱内 `git` 通过环境变量注入 `safe.directory=*` 避免跨用户
  "dubious ownership" 误报。
- **尽力而为,非安全边界**:本实现是命令级隔离的尽力而为方案。受限令牌
  拦截的是"通过沙箱用户执行"的命令对文件系统的越权写入;进程内工具
...
- 若组策略禁止创建防火墙规则,文件隔离仍生效,但网络隔离会降级
  (预置结果会记录该错误)。
- 沙箱不是 VM:不隔离显卡、剪贴板等同会话资源,也不保证防数据泄漏。
